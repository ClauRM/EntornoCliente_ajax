...
//variables globales de array
var arrayURL; //array de urls
var arrayNombres; //array de nombres
var arrayDescripciones; //array de descripciones
...
function almacenaJson(datosJSON) {
    try {
        //array de objetos leido contenidos en el json
        arrayURL = [];

        //recorremos el objeto con un for para almanacenar todas las urls en un array
        for (let i = 0; i < datosJSON.coches.length; i++) {
            url = datosJSON.coches[i].coche; //esta parseado por lo que unicamente accedemos al contenido de la etiqueta "coche" de la posicion i 
            //console.log(datosJSON.coches[i].coche); //para pruebas
            arrayURL.push(url); //aniade la url al arrayURLs
        }

        //console.log("arrayURL : " + arrayURL); //para pruebas
    } catch (error) {
...
function almacenaXML() {
    try {
        var datos = [];//array local para almacenar la coleccion de datos del fichero xml
        var dato; //variable para almacenar el dato que se esta leyendo en un momento determinado con sus propiedades

        arrayNombres = []; //array global para almacenar la propiedad nombre del xml
        arrayDescripciones = []; //array global para almacenar la propiedad descripcion del xml
